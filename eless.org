# Time-stamp: <2021-07-01 16:05:06 gritsenko_yv>
...
(global-set-key (kbd "C-c C-m") (function execute-extended-command))
(global-set-key (kbd "M-v") (function scroll-down-command))
...
(let* ((cfg-file "elesscfg")
  ...
  (unless (load cfg-path :noerror)
    (if (eq system-type (quote windows-nt))
        (load-theme (quote tsdh-dark) :no-confirm)
      (load-theme (quote whiteboard) :no-confirm))
    ;; The tango-dark theme is good except for the bright yellow hl-line face
    (custom-theme-set-faces
     (quote user)
     (if (eq system-type (quote windows-nt))
         (quote (hl-line ((t (:background "blue")))))
       (quote (hl-line ((t (:background "lightgray"))))))
